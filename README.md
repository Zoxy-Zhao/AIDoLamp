...
| 组成 | 内容 | 当前状态 |
|------|------|----------|
| [六轴运动算法](src/motion6dof/) | FK / 几何解析 IK、分支选解、工作空间投影、照射姿态规划与六关节轨迹 | 提供独立算法演示；本次修改完成数值测试，未复核硬件执行 |
...
演示使用合成的书本与人脸目标，输出六关节角、轨迹及 FK 校验残差。模型定义、公式、测试范围和可选绘图命令见[六轴算法说明](docs/six-axis-motion.md)。本次新增六轴模块进行了数值验证，本次修改未复核真机运行；下方介绍仓库保留的四轴原型代码与实物资料。
...
**性能指标（作者提供，待复核）**：手势识别延迟 500 ms → 200 ms。测试条件与原始计时数据尚未随仓库提供，该指标不属于本次六轴算法数值测试结果。
