...
- **关节数量**：本页所述原型 IK、上位机角度接口及下位机执行分支对应四轴；新增六轴模块的命令行演示不调用此四路串口接口。作者确认项目已接入硬件，本次代码核对未复核六轴硬件适配。
...
- **性能口径**：作者提供的待复核指标为手势识别延迟 500 ms → 200 ms，仓库尚未附测试条件及原始计时数据。模型推理耗时与包含图像采集、模式调度及执行的端到端延迟应分别统计。
